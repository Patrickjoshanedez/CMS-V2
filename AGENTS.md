...
*   **Phase 1: Capstone 1 (Title Defense & Proposal Pre-Scan):** Proponents draft 1..10 title proposals tagged with SDGs (1..17). Live archive cosine similarity pre-scan backed by `BAAI/bge-m3` (1024-dim dense + sparse vector core). Proposal defense hearing rubrics determine title approval (`titleStatus = 'approved'`).
*   **Phase 2: Capstone 2 (Chapters 1–3 Manuscript & Midterm Defense):** Chapters 1–3 uploaded. Plagiarism Scan v1 (Winnowing + `BAAI/bge-m3` vector scoring, `< 25%`). Midterm defense evaluation. Action Done Matrix (`ADM v1`) logs panel remarks and multi-signatory digital sign-off.
...
*   **Archival Ingestion & OCR Auto-Fill Engine:** Retrospective capstone uploads process full manuscripts and journals via `PaddleOCR-VL (0.9B)` microservice (`cms-ocr-engine:8000`) and BullMQ asynchronous queue (`document-extraction`) with Redis caching and real-time Socket.IO progress streaming, with resilient fallback to in-process `pdf-parse` (`ocrStatus: 'degraded'`).
...
*   **Mandatory Unified Sophisticated Document Reader Contract:** All document reading, viewing, revision inspection, and manuscript verification across BukSU CMS-V2 MUST universally use the canonical `SophisticatedDocumentViewer` format (`client/src/components/documents/SophisticatedDocumentViewer.jsx`). Fragmented, isolated, or raw document viewers (such as ad-hoc text renderers or `PaginatedDocumentViewer`) are strictly prohibited to ensure 100% visual and institutional consistency. The canonical viewer format MUST preserve all original features intact: Document Identity Bar (chapter title, version badge, file type badge, originality compliance badge, filename, filesize), `Revision Diff (+/-)` mode with multi-granularity diffing (Word Diff, Sentence Diff, Line Diff, revision search, deletion markers, comment markers), high-fidelity `docx-preview` OOXML rendering, PDF stream viewing, Zoom controls, Details metadata drawer, direct file download, and seamless Fullscreen/Maximize expansion. Both embedded inline (`embedded={true}`) and modal dialog presentations must adhere to this unified format.
...
16. **Two-Minute Test Timeout & Hanging Diagnostic Protocol:** Any test suite, Playwright visual audit, or automated command that exceeds **120 seconds (2 minutes)** is strictly flagged as a runaway or hanging process. The agent MUST immediately terminate/kill the task, halt retries, and perform a root-cause diagnostic analysis before re-running:
...
17. **ASDLC Multi-Scenario 3-Layer Task Architecture:**
...
18. **Mandatory Unified Sophisticated Document Reader Contract & Zero-Regression Rule:**
    All document reading across BukSU CMS-V2 (including submission review pages, chapter review panels, detail views, and document tabs) MUST universally use the canonical `SophisticatedDocumentViewer` format (`client/src/components/documents/SophisticatedDocumentViewer.jsx`). Ad-hoc or simplified viewers (`PaginatedDocumentViewer`, raw `<pre>`, isolated HTML dumps) are strictly banned. The unified reader MUST maintain all institutional capabilities:
    *   **Document Identity Bar:** Displays chapter title, version badge (`v{N}`), file format badge (`Word Document` / `PDF Manuscript`), originality compliance badge (`{N}% Original`), filename, and human-readable file size.
    *   **Revision Diff (+/-) Studio:** First-class revision comparison (`v{N-1} → v{N}`) featuring Word Diff, Sentence Diff, Line Diff, revision search input, deletion marker toggles, comment markers, and word addition/subtraction counters.
    *   **High-Fidelity Document Rendering:** Client-side OOXML rendering via `docx-preview` preserving true Word margins, fonts, headings, indentation, and tables, alongside native iframe PDF streaming.
    *   **Interactive Toolbar:** Zoom controls (`-`, `%`, `+`), Details metadata drawer, direct file download (`?download=true`), and responsive Fullscreen / Maximize expansion.
    *   **Dual Presentation Standard:** Supports both embedded inline page flow (`embedded={true}`) and modal dialog presentation (`open={true}`) with identical visual and functional fidelity.
