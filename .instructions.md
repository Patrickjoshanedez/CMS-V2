...
#### 2D.1 Public Internet Exposure Gate (Docker + ngrok, Fail Closed)

- **Hard Stop:** Any Docker/ngrok command that would expose CMS publicly must be blocked unless all gate checks pass with explicit evidence.
- **Production Compose Only:** Public exposure must run through production compose (`docker-compose.prod.yml`) only.
- **Secret Hygiene:** No tracked plaintext production secrets and no secret env values baked into image build context.
- **No Placeholder Secrets:** Placeholder/default secrets are forbidden in public profile.
- **Rate Limit Proof:** Effective rate limiting must be active globally and for authentication routes.
- **Ingress Auth + Domain Policy:** ngrok ingress must enforce OAuth/basic auth and explicit domain policy.
- **Exact CORS Origins:** Public URL must be configured as exact allowed origins (no wildcard/ambiguous values).
- **Proxy/Cookie Compatibility:** `trust proxy` must align with secure and `SameSite` cookie behavior.
- **No LocalStack/Test Storage:** LocalStack/test object storage is forbidden for public production exposure.
- **Least Privilege Runtime:** Apply container runtime least-privilege controls where feasible.
- **Evidence Map Contract:** Verification summary must include `Public Internet Exposure Gate: PASS` and `GATE-1:` through `GATE-10:` lines. Missing, placeholder/default-like, or off-topic evidence fails closed.
