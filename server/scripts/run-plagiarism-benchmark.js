...
function computeMetrics(groundTruthSpans, detectedSpans, documentLength) {
  ...
  for (let i = 0; i < documentLength; i++) {
    if (truthArray[i] && detectedArray[i]) tp++;
    else if (!truthArray[i] && detectedArray[i]) fp++;
    else if (truthArray[i] && !detectedArray[i]) fn++;
  }
  ...
  return { precision, recall, f1, granularity: 1, plagDet: f1 };
}
...
function computeSourceRecall(groundTruthSourceIds, detectedSourceIds) {
  const truth = new Set(
    (Array.isArray(groundTruthSourceIds) ? groundTruthSourceIds : []).map(String),
  );
