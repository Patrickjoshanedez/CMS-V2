...
export async function generateEvaluationReportPdf(evaluationId) {
  ...
  const project = await Project.findById(evaluation.projectId._id || evaluation.projectId)
    .populate('teamId');
  ...
  // Project Info Table
  page.drawText('PROJECT DETAILS', { x: 40, y, size: 11, font: fontBold, color: rgb(0.1, 0.2, 0.4) });
  y -= 15;
  ...
  page.drawText('CRITERION', { x: 45, y: y - 12, size: 9, font: fontBold, color: rgb(0.1, 0.2, 0.4) });
  ...
  page.drawText('REMARKS', { x: 480, y: y - 12, size: 9, font: fontBold, color: rgb(0.1, 0.2, 0.4) });
  ...
  (evaluation.criteria || []).forEach((c, idx) => {
    ...
    page.drawText(String(c.maxScore ?? '-'), { x: 385, y: y - 10, size: 8.5, font, color: rgb(0.2, 0.2, 0.2) });
    page.drawText(String(c.score ?? '-'), { x: 435, y: y - 10, size: 8.5, font: fontBold, color: rgb(0.1, 0.3, 0.6) });
    page.drawText(c.comment ? (c.comment.length > 20 ? `${c.comment.substring(0, 18)}..` : c.comment) : '-', {
      x: 480,
      y: y - 10,
      size: 8,
      font,
  ...
  page.drawText('TOTAL COMPOSITE SCORE:', { x: 45, y: y - 14, size: 10, font: fontBold, color: rgb(0.1, 0.2, 0.5) });
  ...
  page.drawText('PLAGIARISM & SIMILARITY AUDIT', { x: 40, y, size: 11, font: fontBold, color: rgb(0.1, 0.2, 0.4) });
  y -= 16;

  const plagScore = latestSubmission?.plagiarismResult?.overallScore ?? latestSubmission?.originalityScore ?? 100;
  const simPct = (100 - plagScore).toFixed(1);

  page.drawText(`Latest Manuscript: ${latestSubmission?.fileName || 'No submission recorded'} (Version ${latestSubmission?.version || 1})`, {
    x: 45,
    y,
    size: 9,
    font,
  ...
  page.drawText(`Originality Score: ${plagScore}% | Similarity Overlap: ${simPct}% (Checked: ${latestSubmission?.plagiarismResult?.processedAt ? new Date(latestSubmission.plagiarismResult.processedAt).toLocaleDateString() : 'N/A'})`, {
    x: 45,
    y,
    size: 9,
    font: fontBold,
  ...
  // Evaluator Comments
  page.drawText('EVALUATOR COMMENTS & RECOMMENDATIONS:', { x: 40, y, size: 10, font: fontBold, color: rgb(0.1, 0.2, 0.4) });
  y -= 14;
