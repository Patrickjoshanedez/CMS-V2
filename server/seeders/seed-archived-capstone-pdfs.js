...
const ensureArchivedSubmissionPdf = async ({
  ...
  const nextVersion =
    explicitVersion || (await Submission.countDocuments({ projectId: project._id, type })) + 1;
  const sourceStem = sourceFileName ? toSlug(path.parse(sourceFileName).name) : null;
