...
process.env.PDF_METADATA_REVIEW_GATE_ENABLED =
  process.env.PDF_METADATA_REVIEW_GATE_ENABLED || 'true';
...
function compareYear(expected, actual) {
  if (!expected) return { match: true, reason: 'not_scored' };
  return {
    match: Number(expected) === Number(actual),
    reason: Number(expected) === Number(actual) ? 'exact' : 'mismatch',
  };
}
...
function compareAuthors(expected, actual) {
  if (!Array.isArray(expected) || expected.length === 0)
    return { match: true, reason: 'not_scored' };
  ...
  return {
    match: allMatched,
    reason: allMatched ? 'contains_all_expected' : 'missing_expected_author',
  };
}
...
async function runTest() {
  ...
  for (const doc of docs) {
    ...
    const comparisons = {
      ...
      publicationVenue: compareString(
        doc.publicationVenue,
        extracted.publicationVenue || extracted.venue,
      ),
    };
    ...
    for (const [field, comparison] of Object.entries(comparisons)) {
      ...
      textOutput.push(
        `Field ${field}: ${comparison.match ? 'PASS' : 'FAIL'} (${comparison.reason})`,
      );
    }

    const requiresReview =
      Boolean(extracted.review?.required) ||
      Object.values(comparisons).some((entry) => !entry.match);
    if (requiresReview) {
    ...
    textOutput.push(
      extracted.abstract?.substring(0, 500) +
        ((extracted.abstract?.length || 0) > 500 ? '...' : ''),
    );
